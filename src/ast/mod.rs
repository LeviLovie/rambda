...
impl fmt::Display for ReductionType {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ReductionType::BetaReduction(var) => write!(f, "β({})", var),
            ReductionType::AlphaConversion(from, to) => write!(f, "α({} -> {})", from, to),
            ReductionType::ContextualReduction(var) => write!(f, "C({})", var),
            ReductionType::NoReduction => write!(f, "No reduction"),
        }
    }
}
...
impl Expr {
    ...
    fn substitute_once(&self, var: &str, replacement: &Expr) -> (Expr, ReductionType) {
        match self {
            Expr::Var(name) if name == var => (
                replacement.clone(),
                ReductionType::BetaReduction(var.to_string()),
            ),
            Expr::Var(_) => (self.clone(), ReductionType::NoReduction),
            Expr::Abs(param, body) => {
                if param == var {
                    (self.clone(), ReductionType::NoReduction)
                    ...
                    (
                        ...
                        ReductionType::AlphaConversion(param.clone(), fresh),
                    )
                } else {
    ...
    pub fn eval_step(&self) -> (Expr, ReductionType) {
        match self {
            Expr::Apl(e1, e2) => {
                ...
                if red1 != ReductionType::NoReduction {
                    return (
                        ...
                        ReductionType::ContextualReduction("left".to_string()),
                    );
                }
                ...
                if red2 != ReductionType::NoReduction {
                    return (
                        ...
                        ReductionType::ContextualReduction("right".to_string()),
                    );
                }
            ...
            Expr::Abs(param, body) => {
                ...
                if red != ReductionType::NoReduction {
    ...
    pub fn eval_full(&self) -> (Expr, Vec<ReductionType>) {
        let mut reductions = Vec::new();
        let mut expr = self.clone();
        while !expr.is_normal_form() {
            let (next_expr, reduction_type) = expr.eval_step();
            if reduction_type == ReductionType::NoReduction {
                break;
            }
            expr = next_expr;
    ...
    pub fn simplify_numbered_vars(&self) -> (Expr, Vec<ReductionType>) {
        let mut reductions = Vec::new();
        let mut expr = self.clone();
        match self {
            Expr::Var(name) => {
                if let Some((base, num)) = Self::split_name_number(name) {
                    if num == 1 {
                        ...
                        reductions.push(ReductionType::AlphaConversion(name.clone(), base));
                    } else {
                        let new_name = format!("{}_{}", base, num - 1);
                        if self.is_free_in(&new_name) {
                            expr = Expr::Var(new_name.clone());
                            reductions.push(ReductionType::AlphaConversion(name.clone(), new_name));
                        }
                    }
                }
            }
                ...
                if let Some((base, num)) = Self::split_name_number(&param) {
                    if num == 1 {
                        ...
                        if !body.is_free_in(&base) {
                            ...
                            reductions
                                .push(ReductionType::AlphaConversion(param.clone(), base.clone()));
                            param = base;
                        }
                    }
                }
